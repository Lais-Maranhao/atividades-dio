...
def depositar(saldo, valor, extrato, /):
    if valor > 0:
        saldo += valor
        extrato += f"*** Depósito feito no valor de: R$ {valor:.2f} realizado com sucesso! ***\n"
        print(f'*** Seu novo saldo é: R$ {saldo} ***')
    else:
        print("\n XXX Valor inserido é inválido. Operação falhou. Tente novamente. XXX")
...
def criar_usuario(usuarios):
    cpf = input("Informe o CPF do titular da conta (somente número): ")
...
def criar_conta(agencia, numero_conta, usuarios):
    ...
    print("\nXXX Usuário não encontrado, não foi possível criar uma nova conta! XXX")
